...
const func = async function ({ getNamedAccounts, deployments, ...hre }) {
    ...
    // Initialize implementation (skip if resuming after partial deploy — impl already init on-chain)
    const pool = await (0, helpers_1.getPool)(poolArtifact.address);
    try {
        await pool.callStatic.initialize(addressesProviderAddress);
    }
    catch (e) {
        const errText = (e && (e.reason || e.message)) || (e && e.error && e.error.message) || String(e);
        if (errText.includes("already been initialized")) {
            console.log("[Prydox] Pool implementation already initialized, skipping");
            return;
        }
        throw e;
    }
