...
const initReservesByHelper = async (reservesParams, tokenAddresses, aTokenNamePrefix, stableDebtTokenNamePrefix, variableDebtTokenNamePrefix, symbolPrefix, admin, treasuryAddress, incentivesController) => {
    ...
    const protocolName = poolConfig.ProtocolName || "Aave";
    const usePReceipt = poolConfig.ReceiptTokenSymbolPrefix != null && poolConfig.ReceiptTokenSymbolPrefix !== "";
    for (let i = 0; i < reserveSymbols.length; i++) {
        ...
        const aTokenSymbol = usePReceipt
            ? `${poolConfig.ReceiptTokenSymbolPrefix}${reserveSymbols[i]}`
            : `a${symbolPrefix}${reserveSymbols[i]}`;
        const aTokenName = `${protocolName} ${aTokenNamePrefix} ${reserveSymbols[i]}`.replace(/\s+/g, " ").trim();
        initInputParams.push({
            ...
            aTokenName,
            aTokenSymbol,
            variableDebtTokenName: `${protocolName} ${variableDebtTokenNamePrefix} Variable Debt ${reserveSymbols[i]}`.replace(/\s+/g, " ").trim(),
            variableDebtTokenSymbol: `variableDebt${symbolPrefix}${reserveSymbols[i]}`,
            stableDebtTokenName: `${protocolName} ${stableDebtTokenNamePrefix} Stable Debt ${reserveSymbols[i]}`.replace(/\s+/g, " ").trim(),
